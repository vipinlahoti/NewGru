Grudr.updateScore = function (args) {
  ...
  if (postedAt > now) // if article has been scheduled in the future, don't update its score
    return 0;
  ...
  // We assume that after n days, a single vote will not be powerful enough to affect articles' ranking order.
  // Note: sites whose articles regularly get a lot of votes can afford to use a lower n.

  // n =  number of days after which a single vote will not have a big enough effect to trigger a score update
  //      and articles can become inactive
  var n = 30;
  ...
  if (forceUpdate || scoreDiff > x){
    ...
    // only set a article as inactive if it's older than n days
