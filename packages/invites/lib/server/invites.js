Meteor.methods({
  ...
  inviteUser: function(invitation){
    ...
    var currentUserCanInvite = currentUserIsAdmin || (currentUser.grudr.inviteCount > 0 && Users.can.invite(currentUser));
    ...
    if(user && Users.is.invited(user)){
      ...
      Meteor.users.update(Meteor.userId(), {$inc:{"grudr.inviteCount": -1, "grudr.invitedCount": 1}});
      ...
      if(user){
        ...
        Meteor.users.update(user._id, {
          $set: {
            ...
            "grudr.invitedBy": Meteor.userId(),
            "grudr.invitedByName": Users.getDisplayName(currentUser)
          }
        });
      }
      ...
      var communityName = Settings.get('title','Grudr'),
          ...
          emailProperties = {
            ...
            actionLink : user ? Grudr.utils.getSigninUrl() : Grudr.utils.getSignupUrl(),
      ...
      Meteor.setTimeout(function () {
        Grudr.email.buildAndSend(userEmail, emailSubject, 'emailInvite', emailProperties);
      }, 1);
  ...
  unInviteUser: function (userId) {
    Meteor.users.update(userId, {$set: {"grudr.isInvited": false}});
  }
});
