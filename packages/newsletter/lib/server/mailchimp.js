...
scheduleCampaign = function (campaign, isTest) {
  ...
  if(!!apiKey && !!listId){
		...
		while (subject.length >= 150){
			subject = Grudr.utils.trimWords(subject, wordCount);
			wordCount--;
		}
    ...
    try {
      ...
      var campaignOptions = {
        ...
        options: {
          ...
          from_name: Settings.get('title')+ ' Top Articles',
        },
        content: {
          html: campaign.html,
          text: text
      ...
      // if this is not a test, mark articles as sent
      if (!isTest)
        var updated = Articles.update({_id: {$in: campaign.articleIds}}, {$set: {scheduledAt: new Date()}}, {multi: true})

      // send confirmation email
      var confirmationHtml = Grudr.email.getTemplate('emailDigestConfirmation')({
        time: scheduledTime,
        newsletterLink: mailchimpCampaign.archive_url,
        subject: subject
      });
      Grudr.email.send(defaultEmail, 'Newsletter scheduled', Grudr.email.buildTemplate(confirmationHtml));
