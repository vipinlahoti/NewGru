...
Users.addField([
  {
    fieldName: 'grudr.newsletter.showBanner',
    fieldSchema: {
      label: 'Show banner',
      type: Boolean,
      optional: true,
  ...
  {
    fieldName: 'grudr.newsletter.subscribeToNewsletter',
    fieldSchema: {
...
Settings.addField([
  ...
  {
    fieldName: 'articlesPerNewsletter',
    fieldSchema: {
      type: Number,
      optional: true,
      autoform: {
...
// create new "campaign" lens for all articles from the past X days that haven't been scheduled yet
Articles.views.add("campaign", function (terms) {
  return {
    find: {
...
// create new "campaign" lens for all questions from the past X days that haven't been scheduled yet
Questions.views.add("campaign", function (terms) {
  return {
    find: {
      scheduledAt: {$exists: false},
      postedAt: {
...
Grudr.modules.add("contentBottom", {
...
Grudr.callbacks.add("profileCompletedAsync", subscribeUserOnProfileCompletion);
