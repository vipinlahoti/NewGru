Template.css.helpers({
  elementColors: function () {
    ...
    _.each(Grudr.colorElements.colorTable, function (elements, color) {
      var properties = {};
      ...
      _.each(properties, function (selector, property) {
        css += selector + "{\n  " + property + ": " + Settings.get(color, Grudr.colorElements.defaultColors[color]) + ";\n}\n";
      });
    });
    return css;
  },
  ...
  hideAuthClass: function () {
    ...
    // hide separator
    if (authMethods.indexOf('email') == -1 || (authMethods.indexOf('facebook') == -1 && authMethods.indexOf('google') == -1)) {
      authClass += ".at-sep, ";
    }
