...
Template.registerHelper('canArticle', function() {
  return Users.can.article(Meteor.user());
});
Template.registerHelper('canComment', function() {
  return Users.can.comment(Meteor.user());
});
Template.registerHelper('canQuestion', function() {
  return Users.can.question(Meteor.user());
});
Template.registerHelper('canAnswer', function() {
  return Users.can.answer(Meteor.user());
});
Template.registerHelper('canStory', function() {
  return Users.can.story(Meteor.user());
});
...
Template.registerHelper('isDoctor', function(user) {
  var user = typeof user === "undefined" ? Meteor.user() : user;
  if (Users.is.doctor(Meteor.user())) {
    return true;
  }
  return false;
});
