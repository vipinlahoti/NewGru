...
Posts.submit = function (post) {
  ...
  if(!post.title)
    throw new Meteor.Error('Please fill in a title');
...
Meteor.methods({
  ...
  submitPost: function(post){
    ...
    if(!hasAdminRights){
      ...
      if(timeSinceLastPost < postInterval)
        throw new Meteor.Error('Please wait' + (postInterval - timeSinceLastPost) + 'seconds before posting again');

      // check that the user doesn't post more than Y posts per day
      if(numberOfPostsInPast24Hours > maxPostsPer24Hours)
        throw new Meteor.Error('Sorry you cannot submit more than' + maxPostsPer24Hours + 'posts per day');
  ...
  editPost: function (modifier, postId) {
    ...
    if (!user || !Users.can.edit(user, post)) {
      throw new Meteor.Error('Sorry you cannot edit this post');
    }
  ...
  deletePostById: function(postId) {
    ...
    if(!Meteor.userId() || !Users.can.editById(Meteor.userId(), post)) throw new Meteor.Error('You need permission to edit or delete a post');
