 ...
 * @param {Object|string} userOrUserId - The user or their userId
 * @param {Object} document - The document to check (article, comment, user object, etc.)
 */
Users.is.owner = function (userOrUserId, document) {
  try {
    var user = Users.getUser(userOrUserId);
    if (!!document.userId) {
      // case 1: document is a article or a comment, use userId to check
...
Users.is.invited = function (userOrUserId) {
  try {
    var user = Users.getUser(userOrUserId);
    return Users.is.admin(user) || user.grudr.isInvited;
  } catch (e) {
    return false; // user not logged in
  }
};
...
Users.is.doctor = function (userOrUserId) {
  try {
    var user = Users.getUser(userOrUserId);
    return Users.is.admin(user) || user.grudr.isDoctor;
  } catch (e) {
    return false; // user not logged in
  }
};
Users.is.doctorById = Users.is.doctor;
Users.helpers({isDoctor: function () {return Users.is.doctor(this);}});
